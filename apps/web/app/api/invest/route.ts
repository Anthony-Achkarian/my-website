...
export async function POST(request: NextRequest) {
  ...
  let body: { name?: string; email?: string; investorType?: string; amount?: string; message?: string };
  try {
  ...
  const { name, email, investorType, amount, message } = body;

  if (!name || !email || !investorType || !message) {
    return NextResponse.json({ error: "Name, email, investor type, and message are required." }, { status: 400 });
  }
  ...
  try {
    await resend.emails.send({
      ...
      replyTo: email,
      subject: `New Investor Inquiry: ${name}${amount ? ` — ${amount}` : ""}`,
      html: `
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Inter',sans-serif;max-width:600px;margin:0 auto;background:#0a1628;color:#ffffff;padding:40px;border-radius:16px;">
          <div style="margin-bottom:28px;padding-bottom:24px;border-bottom:1px solid rgba(255,255,255,0.08);">
            <p style="font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#60a5fa;margin:0 0 4px;">Ark Industries — Invest</p>
            <h1 style="font-size:22px;font-weight:800;margin:0;color:#ffffff;">New Investor Inquiry</h1>
          </div>

          <table style="width:100%;border-collapse:collapse;">
            <tr>
              <td style="padding:14px 16px 14px 0;border-bottom:1px solid rgba(255,255,255,0.07);vertical-align:top;width:120px;">
            ...
            <tr>
              <td style="padding:14px 16px 14px 0;border-bottom:1px solid rgba(255,255,255,0.07);vertical-align:top;">
                <span style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#64748b;">Investor Type</span>
              </td>
              <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.07);color:#a78bfa;font-weight:700;font-size:15px;">${investorType}</td>
            </tr>
            <tr>
              <td style="padding:14px 16px 14px 0;border-bottom:1px solid rgba(255,255,255,0.07);vertical-align:top;">
                <span style="font-size:11px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:#64748b;">Intended Amount</span>
              </td>
              <td style="padding:14px 0;border-bottom:1px solid rgba(255,255,255,0.07);color:#ffffff;font-weight:600;font-size:15px;">
                ${amount ? amount : '<span style="color:#64748b;font-weight:400;">Not specified</span>'}
              </td>
            </tr>
            <tr>
          ...
          <p style="margin-top:28px;color:#475569;font-size:11px;line-height:1.6;">
            Submitted via <a href="https://arkindustriestech.com/invest" style="color:#64748b;">arkindustriestech.com/invest</a>
          </p>
        </div>
      `,
    });
    ...
    return NextResponse.json({ error: "Failed to submit your inquiry. Please try again." }, { status: 500 });
  }
}
